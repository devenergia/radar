...
class TestInterrupcaoRepositoryContrato:
    ...
    def mock_repository(self) -> AsyncMock:
        ...
        mock = AsyncMock(spec=InterrupcaoRepository)
        return mock

    @pytest.mark.asyncio
    async def test_buscar_ativas_deve_retornar_lista_de_interrupcoes(
        self,
        ...
        resultado = await mock_repository.buscar_ativas()
    ...
    @pytest.mark.asyncio
    async def test_buscar_por_municipio_deve_aceitar_codigo_ibge(
        self,
        mock_repository: AsyncMock,
        ...
        resultado = await mock_repository.buscar_por_municipio(codigo_ibge_boa_vista)
    ...
    @pytest.mark.asyncio
    async def test_buscar_por_conjunto_deve_aceitar_id_conjunto(
        self,
        mock_repository: AsyncMock,
        interrupcao_programada: Interrupcao,
    ) -> None:
        ...
        resultado = await mock_repository.buscar_por_conjunto(1)

        mock_repository.buscar_por_conjunto.assert_called_once_with(1)
        assert isinstance(resultado, list)

    @pytest.mark.asyncio
    async def test_buscar_historico_deve_aceitar_periodo(
        self,
        mock_repository: AsyncMock,
        ...
        resultado = await mock_repository.buscar_historico(data_inicio, data_fim)

        mock_repository.buscar_historico.assert_called_once_with(data_inicio, data_fim)
        assert isinstance(resultado, list)

    @pytest.mark.asyncio
    async def test_buscar_ativas_deve_retornar_lista_vazia_quando_nao_ha_interrupcoes(
        self,
        mock_repository: AsyncMock,
    ) -> None:
