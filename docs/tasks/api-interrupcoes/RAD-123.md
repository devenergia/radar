# RAD-123: Rate Limiting (12 req/min)
...
Implementar rate limiting de 12 requisicoes por minuto conforme especificacao ANEEL.

## IMPORTANTE: Requisitos ANEEL

- **Limite:** 12 requisicoes por minuto por IP/API Key
...
class RateLimitMiddleware(BaseHTTPMiddleware):
    ...
    def __init__(self, app, requests_per_minute: int = 12):
        super().__init__(app)
        ...
        if not self._check_rate_limit(client_id):
            return JSONResponse(
                ...
                content={
                    ...
                    "mensagem": "Rate limit excedido. Maximo 12 requisicoes por minuto.",
...
def create_app() -> FastAPI:
    ...
    # Middlewares
    app.add_middleware(RateLimitMiddleware, requests_per_minute=12)  # Opcao 2
...
class TestRateLimiting:
    async def test_deve_permitir_12_requisicoes_por_minuto(
        self, client, api_key
    ):
        """12 requisicoes em 1 minuto devem ser permitidas."""
        for i in range(12):
    ...
    async def test_deve_bloquear_13a_requisicao(
        self, client, api_key
    ):
        """13a requisicao deve ser bloqueada com 429."""
        # Faz 12 requisicoes
        for _ in range(12):
            await client.get(
                "/quantitativointerrupcoesativas",
...
- [ ] Rate limiting de 12 req/min implementado
